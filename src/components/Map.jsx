...
function MapSearchBar({ onLocationFound }) {
  ...
  return (
    ...
    >
      <form onSubmit={handleSearch} style={{ display: "flex", gap: "5px", margin: 0 }}>
        <input
          type="text"
...
export default function Map({ data = [], setLatLng, selectedMarker }) {
  const center = selectedMarker || parseLocation(data[0]?.location) || { lat: -7.7956, lng: 110.3695 };

  return (
    <div style={{ position: "relative" }}> {/* Pembungkus agar posisi absolute Search Bar bekerja dengan benar */}
      <MapContainer center={[center.lat, center.lng]} zoom={13} style={{ height: "350px", borderRadius: "8px" }}>
