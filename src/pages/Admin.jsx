...
export default function Admin() {
  ...
  const fetchAllData = async () => {
    ...
    const { data: angkut } = await supabase
      .from("pengangkutan")
      .select("*, warga(nama), courier:profiles(name)");
  ...
  return (
    <div style={{ padding: 20 }}>
      ...
      <div style={{ marginBottom: 15 }}>
        {["peta", "warga", "pembayaran", "pengangkutan", "transaksi"].map(
          (t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
            ...
            {list.bayar.map((p) => (
              <tr key={p.id}>
                ...
                <td style={styles.td}>
                  {p.status !== "Sudah" && (
                    <button
                      onClick={() => updateStatus("pembayaran", p.id, "Sudah")}
                  ...
                  <button
                    onClick={() => removeData("pembayaran", p.id)}
                    style={styles.btn("#dc2626")}
                  >
                    Hapus
                  </button>
          ...
          <tbody>
            {list.angkut.map((a) => (
              <tr key={a.id}>
                ...
                <td style={styles.td}>{a.courier?.name || "-"}</td>
                <td style={styles.td}>{a.status}</td>
                <td style={styles.td}>
                  {a.status !== "Selesai" && (
                    <button
                      onClick={() =>
                        updateStatus("pengangkutan", a.id, "Selesai")
                      }
                      style={styles.btn("#16a34a")}
                    >
                      Selesai
                  ...
                  <button
                    onClick={() => removeData("pengangkutan", a.id)}
                    style={styles.btn("#dc2626")}
                  >
                    Hapus
                  </button>
      ...
      {tab === "transaksi" && (
        ...
        >
          ...
          <p>Klik tombol di bawah untuk melihat laporan transaksi lengkap</p>
          <button
            onClick={() => (window.location.href = "/transaction-report")}
